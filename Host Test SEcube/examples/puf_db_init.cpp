...
int puf_db_init() {
	...
	cout << "Welcome to SEcube PUF DB initialization example!" << endl;
	...
	cout << "\n\n==========" << endl;
	cout << "Request PUFs from board ..." << endl << endl;

	// array where the PUFs retrieved from the board will be stored
	uint32_t PUF_list[PUFNUM];	// PUFNUM 1000

	sleep(30);

	// calling function of L1 API responsible for the communication between the host and the board
	l1->L1GetPUFS((uint32_t*)PUF_list);

	cout << "[Host] board response received successfully" << endl;

	// Store the PUF list on DB(file)
	cout << "[Host] Storing PUFs in DB ..." << endl;
	writePUFS(PUF_list, PUFNUM);
	cout << "[DB] PUF database updated successfully" << endl;
	cout << "----------" << endl;
